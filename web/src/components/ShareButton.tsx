...
import { ShareBancaModal, type ShareProduct } from "./ShareBancaModal";
...
interface Props {
  ...
  shortCode: string;
  className?: string;
  /** Quando fornecido, abre o modal de compartilhamento rico (WhatsApp + cartaz Story) */
  storyData?: ShareStoryData;
}

export function ShareButton({ name, shortCode, className = "", storyData }: Props) {
  const [copied, setCopied] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);

  async function handleShare() {
    // Com dados de story → modal de opções
    if (storyData) {
      setModalOpen(true);
      return;
    }

    // Fallback simples: share nativo ou cópia de link
